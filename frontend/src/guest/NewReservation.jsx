...
const NewReservation = ({ user }) => {
	...
	// Check if we arrived here from "Browse Rooms"
	...
	useEffect(() => {
		// If we don't have a room selected yet, we can't calculate a price
		if (!preSelectedRoom) {
			setTotalPrice(0);
			return;
		}
	...
	return (
		<div className="booking-container page-fade-in">
			<div className="glass-card booking-card">
				<h2 className="text-3xl font-bold mb-2">Book Your Stay</h2>
				{preSelectedRoom ? (
					<p className="mb-6">Reserving <strong>{preSelectedRoom.type} - Room {preSelectedRoom.roomNumber}</strong></p>
				) : (
					<p className="opacity-60 mb-6 text-yellow-500">Please select a room from the Rooms tab to see pricing.</p>
				)}
				...
				<form onSubmit={handleSubmit} className="booking-form">
                    ...
                    {!preSelectedRoom
						? "LKR 0.00"
						: (totalPrice > 0 ? `LKR ${totalPrice.toLocaleString()}` : "Invalid Dates")
					}
                   </span>
					</div>
