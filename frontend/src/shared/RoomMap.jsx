...
const RoomMap = ({userRole}) => {
	...
	const [editFormData, setEditFormData] = useState({
		status: "", price: 0, type: "", available: false
	});
	...
	const getFilteredRoom = (room) => {
		...
		return {
			...room, displayStatus: displayStatus, displayClass: uiClass, isBookable: uiClass === "available"
		};
	};
	...
	const handleStatusChange = (e) => {
		...
		setEditFormData(prev => ({
			...prev, status: newStatus, available: newStatus === "AVAILABLE"
		}));
	};
	...
	return (<div className="live-map-container">
		<div className="map-sidebar">
			<div className="sidebar-top">
				<h1 className="map-title">Floor Plan</h1>
				<div className="floor-selector">
					{Object.keys(floorsData).reverse().map(f => (<button
						key={f}
			...
			{selectedRoom ? (<div className="room-inspector animate-slide-in">
				{!isEditing ? (<>
					...
					<div className="inspector-body">
						<div className="inspector-row"><span>Type:</span>
							<span>{selectedRoom.type || 'Standard'}</span></div>
						<div className="inspector-row"><span>Rate:</span>
							<span>LKR {selectedRoom.price?.toLocaleString() || '0'}</span></div>
						<div className="inspector-row"><span>Capacity:</span>
							<span>{selectedRoom.capacity || 2} Guests</span></div>
						<div className="inspector-row"><span>Status:</span>
							<span>{selectedRoom.displayStatus}</span></div>
					</div>
					...
					<div className="form-group">
						...
						<select name="status" className="inspector-input" value={editFormData.status}
					...
					{canEditFullDetails && (<>
						<div className="form-group"><label>Room Price (LKR)</label>
							<input type="number" name="price" className="inspector-input"
								   value={editFormData.price} onChange={handleFormChange}/>
						</div>
						<div className="form-group"><label>Room Type</label>
							<input type="text" name="type" className="inspector-input"
								   value={editFormData.type} onChange={handleFormChange}/>
						</div>
					</>)}
			...
			</div>) : (<div className="inspector-placeholder">
				...
				{(() => {
					...
					return (<div className="grid-layout">
						...
						<div className="hallway">
							<span>{floorNames[currentFloor].toUpperCase()} FLOOR CORRIDOR</span></div>
