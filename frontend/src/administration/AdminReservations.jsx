...
const AdminReservations = () => {
	...
	// For manual booking guest name preview
	...
	// Helper: Lookup name from loaded data when ID is entered
	const handleIdChange = (id) => {
		...
		// Try to find a guest name from existing bookings to help the admin
	...
	// CREATE HANDLER WITH DATE VALIDATION
	...
	const filteredBookings = bookings.filter(res => {
		...
		return gName.includes(searchTerm.toLowerCase()) ||
			bId.includes(searchTerm) ||
					...
					filteredBookings.map((res) => (
						<div key={res.bookingId} className="glass-card res-card">
							<div className="res-card-header">
								...
								<span className={`res-status-pill ${res.bookingStatus?.toLowerCase().replace('_', '-')}`}>
                               ...
                               LKR {res.totalAmount ? Number(res.totalAmount).toLocaleString() : '0.00'}
								...
								<div className="res-actions">
									{res.bookingStatus === 'CONFIRMED' && (
										<button className="res-btn checkin" onClick={() => handleAction(res.bookingId, 'check-in')}>Check In</button>
									)}
									{res.bookingStatus === 'CHECKED_IN' && (
										<button className="res-btn checkout" onClick={() => handleAction(res.bookingId, 'check-out')}>Check Out</button>
									)}
									<button className="res-btn edit" onClick={() => setEditingBooking(res)}>Edit</button>
								</div>
							</div>
						</div>
					))
				) : (
					<div className="no-data-msg">No bookings found matching your search.</div>
				)}
			</div>
					...
					<div className="modal-content glass-card" onClick={(e) => e.stopPropagation()}>
						...
						<form onSubmit={handleCreateBooking}>
							...
							<div className="modal-actions">
								<button type="button" className="cancel-link" onClick={() => setShowAddModal(false)}>Cancel</button>
				...
				<div className="modal-overlay" onClick={() => setEditingBooking(null)}>
					<div className="modal-content glass-card" onClick={(e) => e.stopPropagation()}>
						...
						<form onSubmit={handleUpdateBooking}>
							...
							<div className="form-row">
								...
								<div className="form-group">
									...
									<input
										...
										onChange={(e) => setEditingBooking({...editingBooking, totalAmount: e.target.value})}
									/>
								</div>
							</div>
							...
							<div className="modal-actions">
								<button type="button" className="cancel-link" onClick={() => setEditingBooking(null)}>Cancel</button>
